...
class TagLookupTest {
    ...
    @Test
    void testValidField() {
        assertEquals(0, validate(new TagLookup("name", Operator.EQ, "one")).size(),
            "The name field should be valid");
    }

    @Test
    void testInvalidField() {
        final TagLookup lookup = new TagLookup("invalid", Operator.EQ, "one");
    ...
    void testValidOperators() {
        assertEquals(0, validate(new TagLookup(FIELD, Operator.EQ, "one")).size(),
            "The constraint EQ should be valid");

        assertEquals(0, validate(new TagLookup(FIELD, Operator.NEQ, "one")).size(),
            "The constraint NEQ should be valid");

        assertEquals(0, validate(new TagLookup(FIELD, Operator.LIKE, "one")).size(),
            "The constraint LIKE should be valid");

        assertEquals(0, validate(new TagLookup(FIELD, Operator.UNLIKE, "one")).size(),
            "The constraint UNLIKE should be valid");
    }

    @Test
    void testInvalidOperator() {
        final TagLookup lookup = new TagLookup(FIELD, Operator.GT, "one");
        final Set<ConstraintViolation<Object>> violations = validate(lookup);
        assertEquals(1, violations.size(), "The field name should be invalid");
        final ConstraintViolation<Object> violation = violations.iterator().next();
    ...
    void testInvalidOperators() {
        assertEquals(1, validate(new TagLookup(FIELD, Operator.GT, "one")).size(),
            "The constraint GT should be invalid");

        assertEquals(1, validate(new TagLookup(FIELD, Operator.LT, "one")).size(),
            "The constraint LT should be invalid");

        assertEquals(1, validate(new TagLookup(FIELD, Operator.GTE, "one")).size(),
            "The constraint GTE should be invalid");

        assertEquals(1, validate(new TagLookup(FIELD, Operator.LTE, "one")).size(),
